...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse pay(Integer userId, Long orderNo) {
        ...
        if (orderNo==null){
            return ServerResponse.creatServerResponseByError("订单号不能为空");
        }
        ...
        return ServerResponse.creatServerResponseByError("下单失败");
    }
    ...
    public ServerResponse pay(Order order) throws IOException {
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl("http://39.96.68.67:8080/order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
                .setGoodsDetailList(goodsDetailList);
        //http://39.96.68.67:8080/order/alipay_callback.do   http://3pwznr.natappfree.cc/order/alipay_callback.do
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format("img/qr-%s.png",//E:/photos/qr-%s.png"
                        response.getOutTradeNo());
                log.info("filePath:" + filePath);
                //将二维码生成序列化到filePath目录下
    ...
    public ServerResponse alipay_callback(Map<String, String> map) {
        ...
        //step2：获取流水号   trade_no
        String trade_no = map.get("trade_no");
        //step3：获取支付状态   trade_status
        String trade_status = map.get("trade_status");
        //step4：获取支付时间   gmt_payment
        String payment_time = map.get("gmt_payment");

        Order order = orderMapper.findOrderByOrderNo(orderNo);
        if (order==null){
            return ServerResponse.creatServerResponseByError("订单"+orderNo+"不是本商品的订单");
        }

        if (order.getStatus()>=Const.OrderStatusEnum.ORDER_PAYED.getCode()){
            //防止支付宝的重复回调
            return ServerResponse.creatServerResponseByError("支付宝重复调用");
        }
        ...
        return ServerResponse.creatServerResponseByError("支付信息保存失败");
    }
    ...
    public ServerResponse query_order_pay_status(Long orderNo) {
        ...
        if (order.getStatus()==Const.OrderStatusEnum.ORDER_PAYED.getCode()){
            return ServerResponse.creatServerResponseBySuccess(null,true);
        }

        return ServerResponse.creatServerResponseBySuccess(null,false);
    }
    ...
    public void closeOrder(String time) {
        ...
        //step：查询订单创建<time的订单
        List<Order> orderList = orderMapper.findOrderByCreatTime(Const.OrderStatusEnum.ORDER_UN_PAY.getCode(), time);
        //如果orderList不为空且长度>0
        if (orderList!=null&&orderList.size()>0){
            for (Order order:orderList) {
